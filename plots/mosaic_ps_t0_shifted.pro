...
for i = 0, size(z,/n_elements)-1 do begin 
   t = interpol(tarr,zarr,z[i])
   tnew = t + shift
...
al_legend, ['Pop. III IMF: 100-260 M!D!9n!X', 't!Ddelay!N=0 yr'], charsize=1.1, background_color=6
...
vline, 7.5, linestyle=2
xyouts, 6.5, 1.0e-4, 'z!Dreion!N', orientation=90.0, charsize=1.5, alignment=0.5
...
;; for i = 0, size(redshift,/n_elements)-1 do begin 
;;    print, i, redshift[i], uint(((redshift[i]-initial_redshift)/dz)-1.0)
;; endfor
...
inew_old = 0 
for i = 0, size(redshift,/n_elements)-1 do begin 
   inew = uint(((redshift[i]-initial_redshift)/dz)-1.0)
   ; print, i, inew, ratio1[inew], ratio2[inew]

   ;; if (inew ne inew_old) then begin 
   ;;    gpi3_lm[inew] = gpi_lm[inew]*ratio1[i] 
   ;;    gpi3_hm[inew] = gpi[inew]*ratio2[i]
   ;; endif
...
oplot, redshift1, gpi3_lm, linestyle=5 
oplot, redshift1, gpi3_hm, linestyle=5, color=2
...
inew_old = 0 
for i = 0, size(z,/n_elements)-1 do begin 
   inew = uint(((z[i]-initial_redshift)/dz)-1.0)

   sfr3_lm[inew] = sfr_tot[inew]*pop3_frac1[i] 
   sfr3_hm[inew] = sfr_tot2[inew]*pop3_frac2[i]

   inew_old = inew 
endfor

oplot, z1, sfr3_lm, linestyle=5
oplot, z1, sfr3_hm, linestyle=5, color=2
