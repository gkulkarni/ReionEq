...
PROGRAM YLD 
  ...
  ! This is an alternate version of main.f90 that uses haloyield.f90
  ! to calculate yields of different IMFs. It compiles with `make yld' and
  ! is indeed like using a bulldozer to kill a cockroach.  But it works.
  ...
  REAL(KIND=PREC) :: A, B, BOUND, C, DFM, DGRF, DLT, DLTHI, DLTLO, &
       ...
       &ejrate_n, ejrate_si, ejrate_mg, ejrate_zn, foo
  ...
  if (comnum /= 5) then 
     write (0, '(a)') 'Usage: ./reion RGNOVD RGNSIZE ZLUM FESC YFOO'
     write (0, '(a)') 'Example: ./reion 4.0 7.746 6.5 0.3 14'
     stop
  end if
...
!!$     end if
     ...
     foo = haloyield_species(10.0_prec, yfoo, 3)
