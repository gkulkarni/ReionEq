...
PROGRAM REION 
  ...
  ! File: main.f90
  ! Cre: 2010-04-07
  ! Mod: $Date: 2012/08/27 09:39:57 $; $Revision: 1.1 $ 

  ! BACKUP NOTE: This backup copy was created on 14 August 2012.  It
  ! reproduces the results that I presented in Mysore.  
  ...
  USE INTERFACES, ONLY : CLUMPFAC, DTDZ, GAMMA_PH, GAMMA_PI, HUBP, &
       ...
       &interpolate2, getsfr2, getsfr3, getjmc, getjmh, haloyield, &
       &rtnewt, newtondelta, newtonfm, igmfposto, ejfrac, outfrac, &
       &haloyield_species, counter, ngammafrac
  IMPLICIT NONE 
  ...
  REAL(KIND=PREC) :: A, B, BOUND, C, DFM, DGRF, DLT, DLTHI, DLTLO, &
       ...
       &sm_pop3, sm_pop2, smc_pop3, smc_pop2, smh_pop3, smh_pop2, st_ngamma
...
!!$     end if
     ...
     do i = 1, n_halocalc
        ...
        if (((mmetal_halosc(i)/mgas_halosc(i))/0.02_prec) < METALLICITY_POP3TRANS) then 
           strpop_halosc(i) = 3 
        else 
           strpop_halosc(i) = 2 
        end if
        ...
        DiscScaleLength = 0.05_prec * (DiscSpinParameter/0.01_prec) * HaloVirialRadius ! kpc
...
!!$        mstardot_insitu = fstar * mgas_halosc(i) / timedyn(z) ! 10^10 M_solar yr^-1

        return_fraction = ejfrac(z)
        mstardot = (1.0_prec-return_fraction)*mstardot_insitu ! 10^10 M_solar yr^-1
...
!!$        end if
        ...
        fout = outfrac(m_halosc(i),z) ! dimensionless
        ! fout = 1.0_prec 
        mgasdot = mgasdot - fout*mstardot_insitu ! 10^10 M_solar yr^-1

        mgas_halosc(i) = mgas_halosc(i) + dz*dtdz(z)*mgasdot ! 10^10 M_solar 

        !----------------------------

        zeta = 0.9_prec * exp(-(m_halosc(i)/1.0e2_prec)/0.3_prec) ! dimensionless 
        mmetaldot = fgas_in*(omega_b/omega_nr)*mdot*xigm_tot - &
             &fout*mstardot_insitu*mmetal_halosc(i)/mgas_halosc(i)  &
             &+ haloyield(z)*mstardot_insitu*(1.0_prec-zeta)
        mmetal_halosc(i) = mmetal_halosc(i) + dz*dtdz(z)*mmetaldot   
        HaloMetalAbundance = (mmetal_halosc(i)/mgas_halosc(i))/0.02_prec 
        ismz_halosc(i) = HaloMetalAbundance 
        ! mmetal_halosc(i) = (mmetal_halosc(i)/mgas_halosc(i))/0.02_prec  

        mCdot = fgas_in*(omega_b/omega_nr)*mdot*xigm_c - &
             &fout*mstardot_insitu*mC_halosc(i)/mgas_halosc(i)  &
             &+ haloyield_species(z,7)*mstardot_insitu*(1.0_prec-zeta)
        mC_halosc(i) = mC_halosc(i) + dz*dtdz(z)*mCdot   

        mFedot = fgas_in*(omega_b/omega_nr)*mdot*xigm_fe - &
             &fout*mstardot_insitu*mFe_halosc(i)/mgas_halosc(i)  &
             &+ haloyield_species(z,14)*mstardot_insitu*(1.0_prec-zeta)
        mFe_halosc(i) = mFe_halosc(i) + dz*dtdz(z)*mFedot   

        mOdot = fgas_in*(omega_b/omega_nr)*mdot*xigm_o - &
             &fout*mstardot_insitu*mO_halosc(i)/mgas_halosc(i)  &
             &+ haloyield_species(z,9)*mstardot_insitu*(1.0_prec-zeta)
        mO_halosc(i) = mO_halosc(i) + dz*dtdz(z)*mOdot   
     ...
     do i = 1, n_halocalc 
        ...
        if (((mmetal_halosh(i)/mgas_halosh(i))/0.02_prec) < METALLICITY_POP3TRANS) then 
           strpop_halosh(i) = 3 
        else 
           strpop_halosh(i) = 2 
        end if
...
!!$        mstardot_insitu = fstar * mgas_halosh(i) / timedyn(z) ! 10^10 M_solar yr^-1
...
!!$        end if
        ...
        fout = outfrac(m_halosh(i),z) ! dimensionless
        ! fout = 1.0_prec 
        mgasdot = mgasdot - fout*mstardot_insitu ! 10^10 M_solar yr^-1
        ...
        zeta = 0.9_prec * exp(-(m_halosh(i)/1.0e2_prec)/0.3_prec) ! dimensionless 
        mmetaldot = fgas_in*(omega_b/omega_nr)*mdot*xigm_tot - &
             &fout*mstardot_insitu*mmetal_halosh(i)/mgas_halosh(i)  &
             &+ haloyield(z)*mstardot_insitu*(1.0_prec-zeta)
        ...
        ! mmetal_halosh(i) = (mmetal_halosh(i)/mgas_halosh(i))/0.02_prec  

        mCdot = fgas_in*(omega_b/omega_nr)*mdot*xigm_c - &
             &fout*mstardot_insitu*mC_halosh(i)/mgas_halosh(i)  &
             &+ haloyield_species(z,7)*mstardot_insitu*(1.0_prec-zeta)
        mC_halosh(i) = mC_halosh(i) + dz*dtdz(z)*mCdot   

        mFedot = fgas_in*(omega_b/omega_nr)*mdot*xigm_fe - &
             &fout*mstardot_insitu*mFe_halosh(i)/mgas_halosh(i)  &
             &+ haloyield_species(z,14)*mstardot_insitu*(1.0_prec-zeta)
        mFe_halosh(i) = mFe_halosh(i) + dz*dtdz(z)*mFedot   

        mOdot = fgas_in*(omega_b/omega_nr)*mdot*xigm_o - &
             &fout*mstardot_insitu*mO_halosh(i)/mgas_halosh(i)  &
             &+ haloyield_species(z,9)*mstardot_insitu*(1.0_prec-zeta)
